...
contract ERC734KeyManager {
    ...
    function initialize(address owner) internal {
        ...
        initialized = true;
        bytes32 key = keccak256(abi.encodePacked(owner));
    ...
    function addManager(address _address) public onlyManagementKeyOrSelf {
    ...
    function removeManager(address _address) public onlyManagementKeyOrSelf {
        bytes32 key = keccak256(abi.encodePacked(_address));
        removeKey(key);
    }
